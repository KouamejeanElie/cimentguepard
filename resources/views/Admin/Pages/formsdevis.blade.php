            ...
            <div class="container-fluid mt-3">
                ...
                <div class="row">
                    <div class="col-lg-12">
                        <div class="card">
                            <div class="card-body">
                                 <p class="alert alert-info">Liste des Demandes de Dévis</p>
                                <div class="active-member">
                                    <div class="table-responsive">
                                        <table class="table table-xs mb-0">
                                            ...
                                            <tbody>
                                             @foreach($listeTotalDevis as $listeTotalDevi)
                                                <tr>
                                                    <td>{{ $listeTotalDevi->nom }}</td>
                                                    <td>{{ $listeTotalDevi->identite }}</td>
                                                    <td>{{ $listeTotalDevi->contact }}</td>
                                                    <td>{{ $listeTotalDevi->emaildevis }}
                                                    </td>
                                                    <td>{{ $listeTotalDevi->service }}</td>
                                                    <td>{{ $listeTotalDevi->message }}</td>
                                                     <td>
                                                        <form action="{{ route('path_delete_admin_devis',$listeTotalDevi->id) }}" onsubmit="return confirm('Etes vous sûre ?');" method="POST">
                                                            {{ csrf_field() }}
                                                            {{ method_field('DELETE') }}
